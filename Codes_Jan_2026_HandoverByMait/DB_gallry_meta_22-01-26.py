...
# Drop tables if they exist to ensure a clean schema recreation
# This is important if schema definitions have changed or if old constraints exist.
# BE CAREFUL: This will delete all data in these tables!
try:
    Gallery.__table__.drop(engine)
    logger.info("Dropped existing 'gallery' table.")
except Exception as e:
    logger.warning(f"Could not drop 'gallery' table (might not exist): {e}")
try:
    MetaData.__table__.drop(engine)
    logger.info("Dropped existing 'meta' table.")
except Exception as e:
    logger.warning(f"Could not drop 'meta' table (might not exist): {e}")
